...
sub checkPostprocessFiles {
    ...
    my  ($RGB_nrrd, $FA_nrrd, $MD_nrrd, $baseline_nrrd, $brain_mask_minc, $IDWI_nrrd, $IDWIbet_nrrd, $tensor_nrrd);
    if ($DTIrefs->{$dti_file}->{'Postproc'}->{'Tool'} eq "DTIPrep") {
        ...
        $IDWIbet_nrrd   =   $DTIrefs->{$dti_file}->{'Postproc'}->{'IDWI_bet'}->{'nrrd'};
        $tensor_nrrd    =   $DTIrefs->{$dti_file}->{'Postproc'}->{'tensor'}->{'nrrd'};

        # Return minc files if all nrrd and minc outputs exist on the filesystem
        if ((-e $RGB_nrrd) && (-e $FA_nrrd) && (-e $MD_nrrd) && (-e $baseline_nrrd) && (-e $IDWI_nrrd) && (-e $IDWIbet_nrrd) && (-e $tensor_nrrd)) {
            ...
            $mri_files->{'Postproc'}{'IDWI_bet'}{'nrrd'}    = $IDWIbet_nrrd;
