...
my $NIAK_FOLDER="/data/preventAD/data/pipelines/NIAK/DATA";

my $t='0';

my @new_T1s=`/usr/bin/find ${DATA_TO_TRANSFER} -type f -name "PreventAD_[0-9][0-9][0-9][0-9][0-9][0-9]_BL_adniT1_[0-9][0-9][0-9].mnc" -mtime $t`;

my @new_RSNs=`/usr/bin/find ${DATA_TO_TRANSFER} -type f -name "PreventAD_[0-9][0-9][0-9][0-9][0-9][0-9]_BL_Resting_[0-9][0-9][0-9].mnc" -mtime $t`;

#my @new_ENCs=`/usr/bin/find ${DATA_TO_TRANSFER} -type f -name "PreventAD_[0-9][0-9][0-9][0-9][0-9][0-9]_BL_Encoding_[0-9][0-9][0-9].mnc" -mtime $t`;

#my @new_RETs=`/usr/bin/find ${DATA_TO_TRANSFER} -type f -name "PreventAD_[0-9][0-9][0-9][0-9][0-9][0-9]_BL_Retrieval_[0-9][0-9][0-9].mnc" -mtime $t`;

#my @new_ASLs=`/usr/bin/find ${DATA_TO_TRANSFER} -type f -name "PreventAD_[0-9][0-9][0-9][0-9][0-9][0-9]_BL_ASL_[0-9][0-9][0-9].mnc" -mtime 0`;

foreach my $new_t1(@new_T1s){
    chomp($new_t1);
    my $t1_name = basename($new_t1);
    print "$t1_name \n";
    if($t1_name =~ /(PreventAD)_([0-9][0-9][0-9][0-9][0-9][0-9])_(BL)_(adniT1)_([0-9][0-9][0-9].mnc)/m) {
        my $DCCID = $2;
        my $visitLabel = $3;
        my $acquisition = "$4$5";
        print "mkdir $DCCID \n";
...
foreach my $new_rsn(@new_RSNs){
    ...
    print "$rsn_name \n";
    if($rsn_name =~ /(PreventAD)_([0-9][0-9][0-9][0-9][0-9][0-9])_(BL)_(Resting)_([0-9][0-9][0-9].mnc)/m) {
        my $DCCID = $2;
        my $visitLabel = $3;
        my $acquisition = "$4$5";
        print "mkdir $DCCID \n";
...
#    print "$enc_name \n";
#    if($enc_name =~ /(PreventAD)_([0-9][0-9][0-9][0-9][0-9][0-9])_(BL)_(Encoding)_([0-9][0-9][0-9].mnc)/m) {
#        my $DCCID = $2;
#        my $visitLabel = $3;
#        my $acquisition = "$4$5";
#        print "mkdir $DCCID \n";
...
#    print "$ret_name \n";
#    if($ret_name =~ /(PreventAD)_([0-9][0-9][0-9][0-9][0-9][0-9])_(BL)_(Retrieval)_([0-9][0-9][0-9].mnc)/m) {
#        my $DCCID = $2;
#        my $visitLabel = $3;
#        my $acquisition = "$4$5";
 #       print "mkdir $DCCID \n";
...
#    print "$asl_name \n";
#    if($asl_name =~ /(PreventAD)_([0-9][0-9][0-9][0-9][0-9][0-9])_(BL)_(ASL)_([0-9][0-9][0-9].mnc)/m) {
#        my $DCCID = $2;
#        my $visitLabel = $3;
#        my $acquisition = "$4$5";
#        print "mkdir $DCCID \n";
