...
my $NIAK_script_tail="/data/preventAD/data/bin/mri/PreventADpipelines/NIAK/niak_script_templates/niak_script_PRE_tail.txt";

my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) =localtime(time);
my $date=sprintf("%4d%02d%02d",$year+1900,$mon+1,$mday);
my $NIAK_new_script="/data/preventAD/data/pipelines/NIAK/scripts/niak_PRE_ASL_fmri_preprocess_".$date.".m";
...
foreach my $subject(@subjects){
    ...
    my @sessions=`/usr/bin/find ${subject} -type d -name "PRE[B,F][L,U][0-9][0-9]"`;# -mtime $t`;
    ...
    foreach my $session(@sessions){
        ...
        next unless ($session_name=~m/^PRE[B,F][L,U][0-9][0-9]$/i); 
